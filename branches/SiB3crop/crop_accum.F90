  ...
  do sibpt = 1,subcount  
     ...
     if (sib(sibpt)%param%biome >= 20) then
        if (sib(sibpt)%diag%tb_indx > 1) then
           ...
           !...multiplied by the number of seconds per each timestep to
           !...convert assim mol/sec to mol; multiplied by 12 to convert
           !...mol to g
           sib(sibpt)%diag%assim_d = sib(sibpt)%diag%tb_assim * &
                time%dtsib * 12.0
        endif
        ...
        if (sib(sibpt)%diag%phen_switch == 1) then
           ...
           if (sib(sibpt)%diag%phen_lai < min_lai_crop) then
...
contains
  ...
  subroutine corn_phen
    ...
    if (sib(sibpt)%diag%phen_switch == 1 .and. &
         (time%doy > sib(sibpt)%diag%pd + 175 .or. &
           (time%doy > sib(sibpt)%diag%pd + 125 .and. &
            sib(sibpt)%param%zlt < min_lai_crop)) &
       ) then
 ...
 !......(each stage given as a range of GDDs below) 
    ...
    elseif (sib(sibpt)%diag%gdd < 1000.0) then
       ...
       sib(sibpt)%diag%alloc(2) = 0.3 + 0.05 * temp1
       sib(sibpt)%diag%alloc(3) = 0.2 + 0.15 * temp1
       sib(sibpt)%diag%alloc(4) = 0.0
    ...
    elseif (sib(sibpt)%diag%gdd < 1260.0) then
       temp1 = (sib(sibpt)%diag%gdd - 1080.0) / 80.
       sib(sibpt)%diag%alloc(1) = 0.3  - 0.05 * temp1
    ...
    if (sib(sibpt)%diag%gdd >= 100.0 .and. sib(sibpt)%diag%gdd < 2730.0) then
    ...
    sib(sibpt)%diag%phen_maintr(2) = sib(sibpt)%diag%cum_wt(2) * 0.27 * temp1
    ...
    sib(sibpt)%diag%phen_growthr(4) = sib(sibpt)%diag%allocwt(4) * 0.384 * coeff
    ...
    sib(sibpt)%diag%tot_biomass = &
         sib(sibpt)%diag%cum_drywt(2) + &
         sib(sibpt)%diag%cum_drywt(3) + &
         sib(sibpt)%diag%cum_drywt(4)

    !---------------------------------------------------
    !Decreasing the vmax during the seed filling stage and senescence down
    !to 60% of original vmax value. This is supported in Crafts-Brandner
    !et al. (Plant Physiol., 1992), Jiang et al. (Plant Physiol., 1993), and
    !Jiang et al. (Photosynthesis Research, 1997)
    !
    !kdcorbin, 02/11
    !---------------------------------------------------
    ...
    !EL...since the SLA for irrigated crops is higher, the following will be
    !EL...used for irrigated corn (very slight increase, as no published data
    !EL...could be found for corn).
    ...
    if (sib(sibpt)%diag%phen_switch == 0 .and. &
         sib(sibpt)%diag%gdd >= 100.0 .and. &
         time%doy >= sib(sibpt)%diag%emerg_d) then
 ...
 !EL...1-roots, 2-leaves,3-stems,4-products (flowers and grains)
    ...
    sib(sibpt)%diag%phen_maintr(2) = sib(sibpt)%diag%cum_wt(2) * 0.38 * temp1
    ...
    !Decreasing the vmax during the seed filling stage and senescence down
    !to 60% of original vmax value. This is supported in Crafts-Brandner
    !et al. (Plant Physiol., 1992), Jiang et al. (Plant Physiol., 1993),
    !and Jiang et al. (Photosynthesis Research, 1997)
    !
    !kdcorbin, 02/11
    !---------------------------------------------------
    ...
    if (sib(sibpt)%diag%pd > 0) then
       ...
       elseif (time%doy < sib(sibpt)%diag%pd + 108) then
          ...
          !EL.. the dead leaf pool
          litter = (sib(sibpt)%diag%cum_drywt(2) - &
                    sib(sibpt)%diag%leafwt_c) * 0.97
       else
          sib(sibpt)%diag%leafwt_c = 0.0001
          litter = sib(sibpt)%diag%cum_drywt(1) + &
               sib(sibpt)%diag%cum_drywt(2) + sib(sibpt)%diag%cum_drywt(3)
       endif
    else
       sib(sibpt)%diag%leafwt_c = sib(sibpt)%diag%cum_drywt(2) * 0.01 
    endif
    ...
    if (sib(sibpt)%diag%leafwt_c < min_lai_crop) then
       sib(sibpt)%diag%phen_LAI = min_lai_crop
    else
       sib(sibpt)%diag%phen_LAI = sib(sibpt)%diag%leafwt_c * 2.0 * 0.025
    endif

    !EL...for irrigated soybean, the SLA is higher; so the LAI for irrigated
    !EL...soils, is as follows.
    ...
    if (sib(sibpt)%diag%phen_switch == 0 .and. &
         ...
         time%doy >= sib(sibpt)%diag%pd + 10) then
       sib(sibpt)%diag%phen_switch = 1
       call set_ti(sib(sibpt))
    endif
  ...
  subroutine wheat_phen
    ...
    if (sib(sibpt)%diag%gdd < 105.0) then
...
!   stop
    ...
    if (sib(sibpt)%diag%gdd > 2300.) then
    ...
    elseif (sib(sibpt)%diag%gdd < 680.0) then
       ...
       sib(sibpt)%diag%alloc(2) = 0.4 - 0.15 * temp1
       sib(sibpt)%diag%alloc(3) = 0.2 + 0.25 * temp1
       sib(sibpt)%diag%alloc(4) = 0.0
    elseif (sib(sibpt)%diag%gdd < 769.0) then
       temp1 = (sib(sibpt)%diag%gdd - 680.0) / 89.0
       sib(sibpt)%diag%alloc(1) = 0.3  + 0.1 * temp1
       sib(sibpt)%diag%alloc(2) = 0.25 - 0.249 * temp1
       sib(sibpt)%diag%alloc(3) = 0.45 + 0.149 * temp1
       sib(sibpt)%diag%alloc(4) = 0.0
       sib(sibpt)%diag%alloc(4) = 0.0
    ...
    elseif (sib(sibpt)%diag%gdd < 1569.0) then
       ...
       if (time%doy > sib(sibpt)%diag%pd) then
          ...
          temp1 = (sib(sibpt)%diag%gdd - 1074.0) / 495.0
          sib(sibpt)%diag%alloc(1)= 0.28 - 0.05 * temp1
          sib(sibpt)%diag%alloc(2) = 0.4 + 0.05 * temp1
    ...
    elseif (sib(sibpt)%diag%gdd == 105.0) then
