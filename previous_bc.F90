...
character*100 filename  ! filename used to read in ndvi data
    ...
    ! copy old time-variant bc values to initial set
    do i = 1, subcount
    ...
    ! read in new boundary condition data
    if( drvr_type == 'single' ) then
	...
	read(32,*)ntest1
        ...
        ! next month's ndvi values
        write (filename, "(a,a1,i4,a3)") trim(param_path), '_', &
            time%nyear, '.nc'
        call read_ndvi(filename, time%nmonth, curndvi,  &
            physfrac, d13cresp)
            
        ! calculate time-variant boundary conditions
        call calculate_td (sib, time%mid_month(time%nmonth), curndvi)

        ! copy current ndvi to previous ndvi 
        do i = 1,subcount
            prevndvi(i) = curndvi(i)
        enddo
    endif

end subroutine new_bc
